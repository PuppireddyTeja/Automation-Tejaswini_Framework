...
namespace BDDAutomation.Pages
{
    ...
    public abstract class PageObject
    {
        ...
        private string pageExpectedTitle;
        private string universalPageTitle;
        ...
        protected IList<IWebElement> GetWebElementListBy(By by) {
            return driver.FindElements(by);
                }

        //Hard wait to sleep system execution
        public void HardWait(int timeInSeconds) {
        ...
        protected IWebElement WaitForElementToBeVisible(string xpath, int seconds)
        {
            string logMsg = "WaitForElementToBeVisible " + xpath.ToString() + " for " + seconds + " seconds top.";
            TestsLogger.Log(logMsg);
        ...
        protected IList<IWebElement> WaitForListOfElementsToBeVisible(By by, int seconds)
        {
            string logMsg = "WaitForElementsToBeVisible " + by.ToString() + " for " + seconds + " seconds top.";
            TestsLogger.Log(logMsg);
        ...
        protected IWebElement WaitForElementToBeClickeable(string xpath, int seconds)
        {
            string logMsg = "WaitForElementToBeClickeable " + xpath.ToString() + " for " + seconds + " seconds top.";
            TestsLogger.Log(logMsg);
            try
            {
                WebDriverWait webDriverWait = new WebDriverWait(this.driver, GetTimeSpanInMilliseconds(seconds));
        ...
        public Boolean isGetTextValueEmpty(By by) {
            return WaitForElementToBeVisible(by, 1).Text.Length > 0 ?
                true
                : false;
        }
       ...
       //Common Pause method, do not use insted of Wait Conditions
        ...
        protected IWebElement GetCurrentElement(By by) {
        ...
        protected void MoveToViewElementandSAndkeys(By by, string value)

        {
            IWebElement element = WaitForElementToBeClickeable(by);
            Actions moveToviewElement = new Actions(driver);
            moveToviewElement.MoveToElement(element).Click().SendKeys(value).Build().Perform();
        }

        protected void ClickElementAndSendkeys(By by, string value)
        ...
        public void JavaScriptScrollIntoView(IWebElement element)
        {
            ((IJavaScriptExecutor)driver).ExecuteScript("arguments[0].scrollIntoView(true);", element);
        }
        public void JavaScriptScrollIntoView(By by)
        ...
        public void JavaScriptClear(By by)
        {
            IWebElement webElement = WaitForElementToBeClickeable(by);
        ...
        public string getDateAsStringToEnter(string plusorminus, int val)

        {
            var req = "";
            var date = DateTime.Now.Day;
            if (plusorminus.ToLower().Equals("plus"))
            {
                req= DateTime.Now.AddDays(val).ToString("yyyy-MM-dd").Split('-')[2];
            }
            else if (plusorminus.ToLower().Equals("minus"))
            {
                req = DateTime.Now.AddDays(-val).ToString("yyyy-MM-dd").Split('-')[2];
            }
        ...
        public string GenerateRandomString(int length)
        {
            ...
            return new string(Enumerable.Repeat(alphanum, length)
                .Select(s => s[random.Next(s.Length)]).ToArray());
        }


        public void SelectElementWithByAndText(By by,string text)

        {
            IWebElement webElement = WaitForElementToBeClickeable(by);
            SelectElement sElement = new SelectElement(webElement);
            sElement.SelectByText(text);
